...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const drawer = document.querySelector('.drawer, .menu-drawer, #NavDrawer, #menu-drawer');
    const burgerBtn = document.querySelector('.mobile-drawer-trigger, .drawer-toggle-btn');
    
    if (!drawer) return;

    // 2. ФУНКЦИЯ ОТКРЫТИЯ (для бургера)
    if (burgerBtn) {
        burgerBtn.addEventListener('click', (e) => {
            e.preventDefault();
            drawer.classList.add('is-visible', 'open', 'active');
    ...
    drawer.addEventListener('click', (e) => {
        const isCloseBtn = e.target.closest('.drawer__close, .js-drawer-close, .drawer-close, .close-icon');
        const isOverlay = e.target === drawer || e.target.classList.contains('drawer-overlay');

        if (isCloseBtn || isOverlay) {
            drawer.classList.remove('is-visible', 'open', 'active');
            document.body.classList.remove('overflow-hidden');
            drawer.setAttribute('aria-hidden', 'true');
        }
    });

    // 4. КЛИКАБЕЛЬНОСТЬ ПОДМЕНЮ (Саб-меню)
    // Находим все ссылки/кнопки, у которых есть подменю
    const subMenuToggles = drawer.querySelectorAll('[data-toggle="collapse"], .drawer__nav-toggle-btn');
    ...
    subMenuToggles.forEach(toggle => {
        toggle.addEventListener('click', (e) => {
            ...
            // Находим ID цели (например, #Submenu-1)
            ...
            if (targetCollapse) {
                ...
                // Закрываем другие подменю (эффект аккордеона)
                drawer.querySelectorAll('.collapse.show, .meganav.show').forEach(openMenu => {
                    if (openMenu !== targetCollapse) {
                ...
                if (isOpen) {
                    ...
                    targetCollapse.style.display = 'block'; // Форсируем видимость
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            drawer.classList.remove('is-visible', 'open');
            document.body.classList.remove('overflow-hidden');
        }
    });
});
